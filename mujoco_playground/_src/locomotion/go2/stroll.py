...
#TODO:
# Contar número de caídas (que no caiga)
# Hacer un dataset para worldmodel

MOV_TYPE = "Omni-directional"
    
class Stroll(go2_base.Go2Env):
  """Learn to stroll."""
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    # cmd = [x, y, yaw]
    # linear velocity in X & Y axis; yaw = angular velocity
    
    cmd = self.getCommand(key2)
  ...
  def _get_reward(
    ...
    return {
        ...
        #"pose": self._reward_pose(data.qpos[7:]), # Reward staying close to upright resting pose
        "torques": self._cost_torques(data.actuator_force),
        "action_rate": self._cost_action_rate(action, info["last_act"], info["last_last_act"]),
        "energy": self._cost_energy(data.qvel[6:], data.actuator_force),
        "feet_slip": self._cost_feet_slip(data, contact, info), # Penalizes feet sliding on ground
        "feet_clearance": self._cost_feet_clearance(data), # Penalizes not raising feet when moving them
        "feet_alternate": self._reward_feet_alternate(info["command"], data), # Prevents "galloping like a horse" behaviour
        #"feet_up": self._cost_feet_up(data), # Prevents "limping" gait by reminding the robot to ground its static feet
        #"feet_height": self._cost_feet_height(info["swing_peak"], first_contact, info), # Prevent feet going too high
  ...
  def _cost_feet_up(self, data: mjx.Data) -> jax.Array:
    feet_vel = data.sensordata[self._foot_linvel_sensor_adr]

    vel_xy = feet_vel[..., :2]
    vel_norm_sq = jp.sum(jp.square(vel_xy), axis=-1)

    foot_pos = data.site_xpos[self._feet_site_id]

    # Measure of altitude of non-moving/static feet
    # The robot should not keep static feet off the ground, as
    # this would result in limping
    foot_up = jp.sum(jp.abs(foot_pos[..., -1]) * jp.exp(-vel_norm_sq))
    return foot_up#jp.exp(-jp.sum(jp.square(diff_1) + jp.square(diff_2)))

  def _reward_feet_alternate(self, commands, data: mjx.Data) -> jax.Array:
    # This prevents "galloping", promoting a more regular walking gate.
    feet_vel = data.sensordata[self._foot_linvel_sensor_adr]

    # Reward not moving the two front and rear feet at the same time
    # and not jumping.
    vel_xyz = feet_vel[..., :3]
    ...
    # Measure of jump-like movement (all feet pushing at the same time)
    all_prod = vel_norm_sq[..., 0] * vel_norm_sq[..., 1] * vel_norm_sq[..., 2] * vel_norm_sq[..., 3]
    error = jp.sum(front_feet_prod + rear_feet_prod + all_prod)

    # Reward moving at the same time the opposite front and rear feet
    # while keeping the other feet pair still
    # Z not included so that feet are not rewarded for moving up and down
    vel_xy = feet_vel[..., :2]
    vel_norm_sq = jp.sum(jp.square(vel_xy), axis=-1)
    # The maximum velocity feet should move is two times the desired velocity
    ...
    # Simply maximizing both opposite_prods would reward moving all feet at once
    # With this, for optimal reward one of both opposite_prod must be zero
    res = jp.sum(jp.abs(opposite_prod_1 - opposite_prod_2))

    return jp.log(1. + res)/10 + jp.exp(-error)
